...
const ActionPanel: React.FC<PanelProps> = ({visible, handleWorkspaceVisibility, setMessage}) => {
    ...
    const removeWorkspace = (id: string) => {
        ...
        setMessage("Select a WS to start")
    }
    ...
    return (
        <div className="mt-2">
                    ...
                    <div className="flex row flex-wrap mb-5 justify-between">
                        ...
                        {workspaces?.available.map((workspace) => (
                            <div key={workspace.id}
                                ...
                                <button onClick={() => {
                                    if (workspaces?.selectedWorkspace === workspace.id) {
                                        setMessage("Select a WS to start")
                                    }
                                    addRenderedWorkspace(workspace.id)
                                }}
            ...
            text-md font-medium mb-2 hover:bg-violet-900">{visible ? "Hide" : "Show "} WS
                            </button>
                            <button onClick={() => {
            ...
            text-md font-medium mb-2 hover:bg-violet-900 ml-2">Remove WS
                            </button>
                        </div>
                    }
                </div>
